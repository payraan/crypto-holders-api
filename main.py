...
# تبدیل نام شبکه به شناسه شبکه
def get_network_id(network: str):
    if network.lower() in NETWORK_MAP:
        return NETWORK_MAP[network.lower()]
    raise HTTPException(status_code=400, detail=f"❌ Unsupported network: {network}. Use one of: {', '.join(NETWORK_MAP.keys())}")
...
async def get_token_info(network: str, address: str):
    ...
    try:
        # استفاده از شناسه شبکه "sol" به جای "solana"
        return await fetch_from_holderscan(f"/v0/{network_id}/tokens/{address}")
...
async def get_holders_breakdown(network: str, address: str):
    ...
    try:
        return await fetch_from_holderscan(f"/v0/{network_id}/tokens/{address}/holders/breakdowns")
    except HTTPException as e:
        raise e
    except Exception as e:
...
async def get_holders_trends(network: str, address: str, period: str = "day"):
    ...
    try:
        return await fetch_from_holderscan(f"/v0/{network_id}/tokens/{address}/holders/deltas")
    except HTTPException as e:
        raise e
    except Exception as e:
...
async def get_top_holders(network: str, address: str, limit: int = 10):
    ...
    try:
        return await fetch_from_holderscan(f"/v0/{network_id}/tokens/{address}/holders", {"limit": limit})
    except HTTPException as e:
        raise e
    except Exception as e:
...
async def test_api_key():
    ...
    """
    try:
        # استفاده از شناسه شبکه "sol" به جای "solana" (USDC token address)
        url = f"{BASE_URL}/v0/sol/tokens/EPjFWdd5AufqSSqeM2qN1xzybapC8G4wEGGkZwyTDt1v/holders"
        params = {"limit": 1}
        response = requests.get(url, headers=HEADERS, params=params)
        
        print(f"🔍 Test request to: {url}")
        print(f"✅ Response status: {response.status_code}")
        print(f"✅ Response content: {response.text[:500]}...")  # نمایش فقط 500 کاراکتر اول
